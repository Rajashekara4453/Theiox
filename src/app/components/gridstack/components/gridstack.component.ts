...
export class GridstackComponent
  implements OnInit, OnDestroy, AfterViewInit, OnChanges, Grid {
  @ContentChildren(GridstackItemComponent) gridstackItems: QueryList<
  ...
  ngOnInit(): void {
    ...
    this._zone.runOutsideAngular(() => {
      ...
      $(this._el.nativeElement).on('added', (evt: any, items: Item[]) => {
        const itemsOfCurrentGrid = this._gridstackService.getGridItems(
          this.generatedId
        );
        const existingItemIds: string[] = (<any>this._gridstack).grid.nodes.map(
          (i) =>
            $(i.el)
              .attr('class')
              .split(/\s+/)
        ...
        const itemsToAdd = existingItemIds.filter(
          (i) => !itemsOfCurrentGrid.some((ei) => ei.generatedId === i)
        );
        itemsToAdd.forEach((i) =>
          this._gridstackService.attachGridItem(this.generatedId, i)
        );
      ...
      $(this._el.nativeElement).on('removed', (evt: any, items: Item[]) => {
        ...
        const itemsToDelete = itemsOfCurrentGrid.filter(
          (i) => !existingItemIds.some((ei) => ei === i.generatedId)
        );
        itemsToDelete.forEach((i) =>
          this._gridstackService.detachGridItemIfExists(i.generatedId)
        );
  ...
  private _handleItemChanges(items: GridstackItemComponent[]): void {
    ...
    const itemsToAdd = items.filter(
      (i) => !itemsOfCurrentGrid.some((w) => w.generatedId === i.generatedId)
    );
    const itemsToRemove = itemsOfCurrentGrid.filter(
      (w) => !items.some((i) => i.generatedId === w.generatedId)
    );
  ...
  private _addItem(item: GridstackItemComponent): void {
    this._zone.runOutsideAngular(() => {
      if (
        this._gridstack.willItFit(
          +item.x,
          +item.y,
          +item.width,
          +item.height,
          this._isValueTrue(item.autoPosition)
        )
      ) {
