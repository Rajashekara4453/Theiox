...
export class GridstackService {
  ...
  removeGrid(grid: Grid) {
    this._grids.splice(
      this._grids.findIndex((g) => g.generatedId == grid.generatedId),
      1
    );
  ...
  removeGridItem(gridItemId: string) {
    this._gridItems.splice(
      this._gridItems.findIndex((gi) => gi.item.generatedId == gridItemId),
      1
    );
  }
}
