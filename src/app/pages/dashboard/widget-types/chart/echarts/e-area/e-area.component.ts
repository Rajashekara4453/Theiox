...
@Component({
  selector: 'kl-e-area',
  templateUrl: './e-area.component.html',
  styleUrls: ['./e-area.component.scss']
})
export class EAreaComponent implements OnInit {
  ...
  createOptions(): any {
    ...
    if(this.chartInfo['cData'].chartData && this.chartInfo['cData'].chartData.series) {
      for(let series of this.chartInfo['cData'].chartData.series) {
        if(series.type) {
          series.type = 'line';
          series['areaStyle'] = {};
        }
