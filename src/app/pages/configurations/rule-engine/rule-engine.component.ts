...
export class RuleEngineComponent implements OnInit {
  ...
  constructor(
    ...
    /* ----------------------------------------------------------------------- */
    this.appService.getAllRuleMetaData().subscribe(
      (res) => {
   ...
   */
  getTableData(data) {
    this.appService.getTableDataAndRowData(data).subscribe(
      (res) => {
  ...
  SaveRule() {
    ...
    if (
      this.deviceRuleArr.Selected_ruleType === 'Manual' ||
      this.deviceRuleArr.Selected_ruleType === 'Real Time'
    ) {
    ...
    } else if (this.deviceRuleArr.Selected_ruleType === 'Schedule') {
    ...
    if (this.isErrorInRuleArr == false && tagFrequencyCheck === true) {
      ...
      this.appService.saveAndEditRuleForm(this.deviceRuleArr).subscribe(
        (res) => {
          if (res.status == 'success') {
  ...
  editRule(id) {
    const objGet = {};
    objGet['rule_engine_id'] = id;

    this.appService.getTableDataAndRowData(objGet).subscribe(
      (res) => {
  ...
  Conf_deleteRule() {
    ...
    this.appService.deleteRuleById(this.ruleListRowID).subscribe(
      (res) => {
        if (res.status == 'success') {
          //console.log("save new rule", res);
          this._toastLoad.toast('success', 'Saved', res.message, true);
  ...
  setCalcSuggestions() {
    ...
    /* --------------------------------------------------------------------------- */
    this.appService.getAllMonacoSuggetion().subscribe(
      (res) => {
        if (res.status == 'success') {
          ...
          this.calcSuggestions = this.calcSuggestions.concat(tagsArray);
          this.appService.getLookupSuggession().subscribe(
            (look_up) => {
